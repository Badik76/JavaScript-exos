...
function isFloat(n){ //fonction pour n'avoir que des chiffres différent de 0.
...
submit.addEventListener("click", function(){ //début de la fonction submit
  if (isFloat(firstNumber.value.replace(",", ".") && secondNumber.value.replace(",", "."))) { //Test du isFloat + remplacement de la , par un .
        alert(firstNumber.value.replace(",", ".")%secondNumber.value.replace(",", ".")); //résultat si bon
      } else { //si la valeur n'est pas vérifié
          alert("Erreur dans la(es) case(s)") //il s'affichera ceci.
  }//fin du else.
